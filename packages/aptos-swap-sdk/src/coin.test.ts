...
describe('Coin', () => {
  ...
  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'KLAY').equals(new Coin(ChainId.TESTNET, ADDRESS_TWO, 18, 'KLAY'))
      ).toBe(false)
    })

    it('false if chain id differs', () => {
      expect(
        new Coin(ChainId.MAINNET, ADDRESS_ONE, 18, 'KLAY').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'KLAY'))
      ).toBe(false)
    })

    it('true if only decimals differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 9, 'KLAY').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'KLAY'))
      ).toBe(true)
    })

    it('true if address is the same', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'KLAY').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'KLAY'))
      ).toBe(true)
    })

    it('true on reference equality', () => {
      const token = new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'KLAY')
    ...
    it('sortBefore', () => {
      ...
      expect(
        new Coin(ChainId.TESTNET, '0x16::pancake::KLAY', 8, '').sortsBefore(

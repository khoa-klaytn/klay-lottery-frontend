...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"KLAY" | "USD">("KLAY");
  const conversionUnit = editingUnit === "KLAY" ? "USD" : "KLAY";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "KLAY" ? "USD" : "KLAY";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "KLAY") {
      ...
      setValues({
        KLAY: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };

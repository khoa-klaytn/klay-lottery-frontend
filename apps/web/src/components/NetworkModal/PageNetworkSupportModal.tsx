import { Button, Modal, Text, Grid, Box, Message, MessageText } from '@sweepstakes/uikit'
  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a Klaytn only feature')}</Text>
        ...
        <Text small>
          {t(
            'Our Pools, Limit, Trading Competition, Lottery and NFTs features are currently available only on Klaytn! Come over and join the community in the fun!',
          )}
        </Text>
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.KLAYTN) : switchNetworkAsync(ChainId.KLAYTN))}
          >
            {t('Switch to %chain%', { chain: 'Klaytn' })}
          </Button>
        ) : (
